...
3. 告诉用户设置好 key 后可使用这些接口：`search` 检索列表、`detail` 获取详情解析、`download` 下载全文 PDF、`health` 检查服务状态。
...
7. 下载时优先使用搜索结果的 `new_url`、`title`、`data_filename`、`data_dbname`、`time`；旧字段 `durl`、`documentName`、`accessionNo`、`database`、`date` 也可直接传。
